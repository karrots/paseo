...
test.describe("provider usage settings", () => {
  test("keeps request counts with their window and distinguishes partial refills", async ({
    ...
    const counts = await card.getByText("0 / 750 requests", { exact: true }).boundingBox();
    const weekly = await card.getByText("Weekly", { exact: true }).boundingBox();
    expect(counts).not.toBeNull();
    expect(weekly).not.toBeNull();
    if (!counts || !weekly) throw new Error("Expected both usage rows to be visible");
    expect(counts.y).toBeLessThan(weekly.y);
